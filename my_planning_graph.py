    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            for node_s in self.s_levels[level]:
                #   adding action_node to current level, if PgNode_a has prenodes that are a subset of the previous S-level
    ...
    def add_literal_level(self, level):
        ...
        for node_a in self.a_levels[level-1]:
            for node_s in node_a.effnodes:
                # Getting the effect nodes for the preceding action and adding them to the level's state nodes
